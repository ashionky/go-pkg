...
func (self *PhoneValidator) Validate(params map[string]interface{}, val reflect.Value, args ...string) (bool, error) {
	pMsg := "[name] is not a phone number"
	pParamsMap := map[string]string{
		"name": params["name"].(string),
	}
	if self.PMsg != "" {
		pMsg = self.PMsg
	...
	if !checkString(val.Kind()) {
		return false, formatError(pMsg, pParamsMap)
	}
	reg := PHONE_REG
	if self.Reg != "" {
		reg = self.Reg
...
func (self *DateValidator) Validate(params map[string]interface{}, val reflect.Value, args ...string) (bool, error) {
	eMsg := "[name] is not a date"
	fmtStr := "Y-m-d"
	eParamsMap := map[string]string{
		"name": params["name"].(string),
	}
	...
	//拼接
	replaceArr := []string{
		"Y", YEAR_REG, "m", MONTH_REF, "d", DAY_REF,
	}
...
func (self *TimeValidator) Validate(params map[string]interface{}, val reflect.Value, args ...string) (bool, error) {
	eMsg := "[name] is not a time"
	fmtStr := "H:i:s"
	eParamsMap := map[string]string{
		"name": params["name"].(string),
	}
	...
	//拼接
	replaceArr := []string{
		"H", HOUR_REF, "i", MINUTE_REF, "s", SECOND_REF,
	}
	replacer := strings.NewReplacer(replaceArr...)
	reg := `^` + replacer.Replace(fmtStr) + `$`
	if !regexp.MustCompile(reg).MatchString(val.String()) {
